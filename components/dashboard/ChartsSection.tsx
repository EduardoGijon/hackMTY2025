    ...
    if (isSimpleMode) {
        ...
        return (
            <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl shadow-sm border p-6">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-6">
                        ...
                        <div className="text-center p-6 bg-green-50 rounded-lg border-2 border-green-200">
                            ...
                            <h4 className="text-lg font-semibold text-green-800 mb-2">Money IN</h4>
                            <div className="text-3xl font-bold text-green-600">
                                ${(data.totalIncome || 0).toLocaleString()}
                            </div>
                            <p className="text-sm text-green-600 mt-1">Money you received</p>
                        </div>

                        {/* Money OUT */}
                        <div className="text-center p-6 bg-red-50 rounded-lg border-2 border-red-200">
                            <TrendingDown className="w-12 h-12 text-red-600 mx-auto mb-3" />
                            <h4 className="text-lg font-semibold text-red-800 mb-2">Money OUT</h4>
                            <div className="text-3xl font-bold text-red-600">
                                ${(data.totalExpenses || 0).toLocaleString()}
                            </div>
                            <p className="text-sm text-red-600 mt-1">Money you spent</p>
                        </div>
                    </div>
                    ...
                    <div className="text-center p-6 bg-blue-50 rounded-lg border-2 border-blue-200">
                        ...
                        <p className="text-sm text-blue-600">
                            {(data.netBalance || 0) >= 0 ? 'You can save it or invest it' : 'You need to spend less or earn more'}
                        </p>
                    </div>
                </div>

                {/* SUPER simple chart: Where does your money come from? */}
                {topIncome.length > 0 && (
                    <div className="bg-white rounded-xl shadow-sm border p-6">
                        <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
                            <Eye className="w-5 h-5 text-green-600" />
                            Where does your money come from? (Top 3)
                        </h3>
                                ...
                                return (
                                    <div key={item.category} className="relative">
                                        ...
                                        <p className="text-xs text-gray-500 mt-1">
                                            Out of every $100 you receive, ${percentage.toFixed(0)} comes from here
                                        </p>
                                    </div>
                                );
                            })}
                ...
                {/* SUPER simple chart: What do you spend most on? */}
                {topExpenses.length > 0 && (
                    <div className="bg-white rounded-xl shadow-sm border p-6">
                        <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
                            <Eye className="w-5 h-5 text-red-600" />
                            What do you spend most on? (Top 3)
                        </h3>
                            ...
                            {topExpenses.map((item: any, index: number) => {
                                ...
                                return (
                                    <div key={item.category} className="relative">
                                        ...
                                        <p className="text-xs text-gray-500 mt-1">
                                            Out of every $100 you spend, ${percentage.toFixed(0)} goes here
                                        </p>
                                    </div>
                                );
                            })}
                        ...
                        <div className="mt-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                            ...
                            <p className="text-yellow-700 text-sm">
                                Your biggest expense is <strong>{topExpenses[0]?.category}</strong> (${topExpenses[0]?.amount.toLocaleString()}).
                                Can you reduce it a bit? Even $500 less would help a lot.
                            </p>
                        </div>
                    </div>
                )}

                {/* Super simple trend: How have you been these months? */}
                    ...
                    <div className="bg-white rounded-xl shadow-sm border p-6">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                            {data.monthlyTrend.slice(-3).map((month: any, index: number) => {
                                ...
                                const monthName = new Date(month.month + '-01').toLocaleDateString('en', { month: 'short' });

                                return (
                                    <div key={month.month} className={`text-center p-4 rounded-lg border-2 ${isPositive ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'
                                        }`}>
                                        ...
                                        <div className="text-xs text-gray-500 mt-1">
                                            {isPositive ? 'You earned' : 'You lost'}
                                        </div>
                                    </div>
                                );
                            })}
                        ...
                        <div className="mt-4 text-center text-sm text-gray-600 flex items-center justify-center gap-2">
                            {data.monthlyTrend.filter((m: any) => m.balance >= 0).length > data.monthlyTrend.length / 2 ? (
                                ...
                                <>
                                    ...
                                    Some months have been tough. What can you change?
                                </>
                            )}
                        </div>
                    </div>
